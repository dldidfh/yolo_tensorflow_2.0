...
class YOLOv1(tf.keras.Model):
    def __init__(self, args):
        ...
        output = Conv2D(5*self.args.box_per_grid + self.args.class_num, kernel_size=(1,1))(x)
...
class YOLOv1_1(tf.keras.Model):
    def __init__(self, args):
        super(YOLOv1_1, self).__init__()
        self.args = args 

        lrelu = tf.keras.layers.LeakyReLU(alpha=0.1)

        input = tf.keras.Input(shape=(self.args.input_scale,self.args.input_scale, 3,))
        x = Conv2D(filters=64, kernel_size=(7,7), strides=(1,1), padding='same', activation=lrelu)(input)
        x = MaxPool2D(pool_size=(2,2), strides=(2,2), padding='same')(x)
        
        x = Conv2D(filters=192, kernel_size=(3,3), padding='same', activation=lrelu)(x)
        x = MaxPool2D(pool_size=(2,2), strides=(2,2), padding='same')(x)

        x = Conv2D(filters=128, kernel_size=(1,1), padding='same', activation=lrelu)(x)
        x = Conv2D(filters=256, kernel_size=(3,3), padding='same', activation=lrelu)(x)
        x = Conv2D(filters=256, kernel_size=(1,1), padding='same', activation=lrelu)(x)
        x = Conv2D(filters=512, kernel_size=(3,3), padding='same', activation=lrelu)(x)
        x = MaxPool2D(pool_size=(2,2), strides=(2,2), padding='same')(x)
        ...
        x = Conv2D(filters=256, kernel_size=(1,1), padding='same', activation=lrelu)(x)
        x = Conv2D(filters=512, kernel_size=(3,3), padding='same', activation=lrelu)(x)
        x = Conv2D(filters=512, kernel_size=(1,1), padding='same', activation=lrelu)(x)
        x = Conv2D(filters=1024, kernel_size=(3,3), padding='same', activation=lrelu)(x)
        x = MaxPool2D(pool_size=(2,2), strides=(2,2), padding='same')(x)
